...
def alphabeta_search(state, game, d=4, cutoff_test=None, eval_fn=None):
    ...
    eval_fn = eval_fn or (lambda game, state: (1 if game.max_to_move(state) else -1)*game.utility(state, game.to_move(state)))
...
def random_player(game, state, display=False):
