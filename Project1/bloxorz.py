...
def next_position(pos, action, reverse_direction=False):
    ...
    if reverse_direction:
        return next_position(pos, reverse_action_dict[action])

    blockpos = get_direction(pos)
    ax = pos[0][0]
    ay = pos[0][1]
    bx = pos[1][0]
    by = pos[1][1]
    if blockpos=="up":
        if action=="U":
            return (ax,ay-2),(bx,by-1)
        if action=="D":
            return (ax,ay+1),(bx,by+2)
        if action=="L":
            return (ax-2,ay),(bx-1,by)
        if action=="R":
            return (ax+1,ay),(bx+2,by)
    if blockpos=="horz":
        if action=="U":
            return (ax,ay-1),(bx,by-1)
        if action=="D":
            return (ax,ay+1),(bx,by+1)
        if action=="L":
            return (ax-1,ay),(ax-1,ay) #upright
        if action=="R":
            return (ax+2,ay),(ax+2,ay) #upright
    if blockpos=="vert":
        if action=="U":
            return (ax,ay-1),(ax,ay-1) #upright
        if action=="D":
            return (ax,ay+2),(ax,ay+2) #upright
        if action=="L":
            return (ax-1,ay),(bx-1,by)
        if action=="R":
            return (ax+1,ay),(bx+1,by)
...
def get_direction(pos):
    ...
    if pos[0][1] == pos[1][1]:
        ...
        raise ValueError("illegal diagonal block position")


if __name__ == '__main__':
    cs210_utils.cs210_mainstartup()
    testboard= Board.read_board(open("testboard.txt"))
    print(testboard.legal_position(((1,2),(2,2))))
